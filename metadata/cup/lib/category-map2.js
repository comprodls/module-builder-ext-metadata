...
function createMapFromFlattened(flattenedMap){
    ...
    for(let id in data) {
        ...
        let currentTax = JSON.parse(JSON.stringify(data[id]));
        let relationships = data[id].relationships;
        delete currentTax.relationships;
        relationships.push(currentTax);
        let transformedTax = {};
        // fill transformedTax.tags with n level heirarchy of taxonomy terms.
        transformedTax = createTree(relationships);
...
function createTree(termsArray){
    ...
    for(let i=0; i< termsArray.length; i++) {
        ...
        termsArray[i]["id"] = termsArray[i]["id"] || termsArray[i]["relationship_id"];
        ...
        if(childParentMap[parentId].length) {
            ...
            for(let i=1; i< childParentMap[parentId].length; i++) {
                ...
                recMap[childParentMap[parentId][i]["id"]].doc["meta"] =  {
                    ...
                    "description": recMap[childParentMap[parentId][i]["id"]].doc.description,
                    "theme": ""
                }

                delete recMap[childParentMap[parentId][i]["id"]].doc.description;
                delete recMap[childParentMap[parentId][i]["id"]].doc.relationship_id;
                delete recMap[childParentMap[parentId][i]["id"]].doc.relationship_type;
                delete recMap[childParentMap[parentId][i]["id"]].doc.direction;
                delete recMap[childParentMap[parentId][i]["id"]].doc.weight;
                // console.log(recMap);
                // call getchildren again for current term childs with updated parameters.
                getChildren(recMap[childParentMap[parentId][i]["id"]].tags, childParentMap[parentId][i]["id"]);   
            }
        }
    }
    return recMap;
